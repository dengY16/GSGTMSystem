...
(function($){
	...
	{	
		...
		if(opt.url!=""){
			if(isNewWin){
				...
				iframeWin.$("#s_"+opt.idFled).dialog(opt);
	...
	$.fn.ldialog=function(options,param){
		options = options || {};
		if(options.ishowButton == true){
			options.buttons= [{
				text:'确定',
				id:'defaultOK',
				iconCls:'icon-save',
				handler:function(){
					var ifrmae = iframeWin.document.getElementById("dialog_showWindow_"+options.idFled).contentWindow;
					ifrmae.submitFrom();
				}
			},{
				text:'取消',
				...
				iconCls:'icon-redo',
				handler:function(){
					iframeWin.$("#s_"+options.idFled).window("close");
				}
			}];
		}else{
			options.buttons= [];
